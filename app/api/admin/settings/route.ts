  ...
  try {
    await sql`
      CREATE TABLE IF NOT EXISTS store_settings (
        ...
        store_name VARCHAR(255) DEFAULT 'Golden Threads',
    ...
    if (storeCount[0].count === 0) {
      await sql`
        ...
        VALUES ('Golden Threads', 'Premium fashion for women and kids', 'info@goldenthreads.com', '+91-9876543210', 'Fashion Street, Mumbai, India', 'INR', 'Asia/Kolkata', 18, 50, 999, '', '', '', '', '')
      `
    }
  ...
  try {
    ...
    return NextResponse.json({
      store: storeSettings[0] || {
        store_name: "Golden Threads",
        store_description: "Premium fashion for women and kids",
        store_email: "info@goldenthreads.com",
  ...
  try {
    ...
    if (store) {
      const result = await sql`
        ...
        SET 
          store_name = ${store.store_name || "Golden Threads"},
          store_description = ${store.store_description || "Premium fashion for women and kids"},
          store_email = ${store.store_email || "info@goldenthreads.com"},
